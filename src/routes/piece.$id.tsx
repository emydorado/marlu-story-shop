...
export const Route = createFileRoute("/piece/$id")({
  ...
  notFoundComponent: () => (
    <div className="max-w-md mx-auto text-center py-32 px-6">
      <h1 className="font-serif text-3xl text-primary mb-4">This piece can't be found.</h1>
      <Link to="/" className="text-sm border-b border-primary/40">Return home</Link>
    </div>
  ),
  errorComponent: () => (
    <div className="max-w-md mx-auto text-center py-32 px-6">
      <p className="font-serif text-2xl text-primary">Something didn't load.</p>
    </div>
  ),
});
...
function PiecePage() {
  ...
  return (
    <article className="max-w-6xl mx-auto px-5 sm:px-8 py-10 sm:py-16">
      <Link to="/" className="text-xs uppercase tracking-[0.25em] text-muted-foreground hover:text-primary transition-colors">
        ← Back
      </Link>
      <div className="grid md:grid-cols-2 gap-10 lg:gap-16 mt-8">
        ...
        <div className="flex flex-col justify-center max-w-md">
          <p className="text-xs uppercase tracking-[0.3em] text-ochre mb-4">Handmade · One of few</p>
          ...
          <p className="mt-8 text-xs italic text-muted-foreground">
            Made by hand. No two pieces are exactly the same.
          </p>
        </div>
      </div>
    </article>
